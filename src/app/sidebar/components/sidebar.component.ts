...
import { isLTR, isIOS, isBrowser } from '../helper/utils';

@Component({
	selector: 'kryptand-sidebar',
	template: `
    <aside #sidebar
      role="complementary"
      [attr.aria-hidden]="!opened"
      [attr.aria-label]="ariaLabel"
      class="sidebar ng-{{opened ? 'opened' : 'closed'}} ng-{{position}} ng-{{mode}}"
      [class.docked]="_isDocked"
      [class.inert]="_isInert"
...
export class Sidebar implements OnInit, OnChanges, OnDestroy {
	...
	@Input()
	position: 'start' | 'end' | 'left' | 'right' | 'top' | 'bottom' = 'start';
	@Input() animate = true;
